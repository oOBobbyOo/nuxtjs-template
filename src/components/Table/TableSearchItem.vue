...
const props = defineProps<TableSearchItemProps>()
...
// 接收 columns 并设置为响应式
const _searchParams = reactive(props.searchParams)
...
const is = computed(() => {
  ...
  const type = `el-${props.column.search?.el}` as keyof typeof elementTypeMap
  return props.column.search?.render ?? elementTypeMap[type]
})
