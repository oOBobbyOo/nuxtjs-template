...
function dragStart(e: MouseEvent) {
  if (!props.isPassing) {
    // 记录开始时间
    startTime.value = getDayjsValueOf()
    isMoving.value = true
    x.value = e.pageX
  }
  emit('handlerMove')
...
// 拖拽结束回调
function dragFinish(e: MouseEvent) {
  if (isMoving.value && !props.isPassing) {
    const diffX = e.pageX - x.value
...
<template>
  ...
  >
    <div ref="progressBar" class="absolute h-0 w-0" :class="{ goFirst2: isOk }" :style="progressBarStyle" />

    <div ref="message" class="drag-message absolute top-0 select-none" :style="textStyle">
      <div
        class="flex-center gap-1" :style="{
    ...
    <div
      ref="handler" class="absolute left-0 top-0 flex-center cursor-move" :class="{ goFirst: isOk }"
      :style="handlerStyle" @mousedown="dragStart"
    >
      <Icon
        :icon="props.isPassing ? successIcon : handlerIcon" class="text-[21px]"
